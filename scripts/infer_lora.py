...
def generate_with_context(model, tokenizer, device, question, context_db):
    """
    Retourne directement la réponse du CSV si la question y figure.
    Sinon, génère une réponse avec le modèle.
    """
    if question in context_db:
        # Question exacte dans le CSV → on retourne la réponse telle quelle
...
if __name__ == "__main__":
    ...
    while True:
        ...
        answer = generate_with_context(model, tokenizer, device, question, context_db)
        print("\n🟩 Réponse :")
        print(answer)
